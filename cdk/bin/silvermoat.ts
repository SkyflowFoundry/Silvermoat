...
const domainName = app.node.tryGetContext('domainName') || process.env.DOMAIN_NAME || 'silvermoat.net';
const createCloudFront = app.node.tryGetContext('createCloudFront') || process.env.CREATE_CLOUDFRONT || 'true';

new SilvermoatStack(app, 'SilvermoatStack', {
  appName,
  stageName,
  apiDeploymentToken,
  uiSeedingMode,
