...
if [ "$VERTICAL" = "all" ] || [ "$VERTICAL" = "fintech" ]; then
  echo "→ Deploying Fintech Stack..."
  cdk deploy "${BASE_STACK_NAME}-fintech" --require-approval never
  echo "✓ Fintech stack deployed"
  echo ""
fi

if [ "$VERTICAL" = "all" ] || [ "$VERTICAL" = "landing" ]; then
  echo "→ Deploying Landing Stack..."
  cdk deploy "${BASE_STACK_NAME}-landing" --require-approval never
  echo "✓ Landing stack deployed"
  echo ""
fi
