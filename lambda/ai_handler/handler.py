...
def handler(event, context):
    ...
    """
    # Get response stream early for error handling
    ...
    try:
        ...
        # Log stream start
        print(f"[STREAM_START] {method} /{path} - RequestId: {context.request_id}")
        ...
        # Flush and log completion
        response_stream.flush()
        print(f"[STREAM_END] {method} /{path} - RequestId: {context.request_id}")
        return None  # Response already written to stream

    except json.JSONDecodeError as e:
        print(f"[ERROR] Invalid JSON: {str(e)} - RequestId: {context.request_id}")
        write_error_chunk(response_stream, "invalid_json", f"Invalid JSON in request body: {str(e)}")
        response_stream.flush()
        return None

    except Exception as e:
        print(f"[ERROR] Internal error: {str(e)} - RequestId: {context.request_id}")
        import traceback
        traceback.print_exc()
