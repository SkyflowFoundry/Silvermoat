...
def handle_customer_chat_streaming(body, storage, response_stream):
    """Handle streaming customer chat request via Function URL"""
    ...
    try:
        ...
        if not customer_email:
            write_error_chunk(response_stream, "customer_email_required", "Customer email is required")
            return
        ...
        # System prompt for customer assistant
        system_prompt = f"You are a helpful AI assistant for Silvermoat Insurance customers. You are assisting the customer with email: {customer_email}. Help them view their policies, check claim status, and review payment history. Use the available tools to search their data. Be friendly, professional, and clear. Only show information for this specific customer."
        ...
        print(f"Customer chat streaming error: {str(e)}")
